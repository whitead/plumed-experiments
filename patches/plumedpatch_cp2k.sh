#!/bin/bash
# PATCH SCRIPT FOR cp2k, new development version from release <xyz>
#
...
function plumed_rename_rule (){
  case "$1" in
  (restraint.c) echo "${1%.c}_main.cpp" ;;
  (*.c) echo "${1%.c}.cpp" ;;
  (*)   echo "$1" ;;
  esac
}
...
function to_do_after_patch () {
  ...
  sed -i 's/ restraint.o/ restraint_main.o/g' src-plumed/plumed.inc
  sed -i 's/\/restraint.cpp/ restraint_main.cpp/g' src-plumed/plumed.inc
