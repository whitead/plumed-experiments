...
# this is strictly dependent on the arch 
#
myarch="openmpi"
...
cd src
mkdir ./USER-PLUMED
cat > ./USER-PLUMED/Install.sh << \EOF
...
EOF
cat > ./USER-PLUMED/Install.csh << \EOF
# Install/unInstall package classes in LAMMPS
...
EOF
cat >./USER-PLUMED/fix_plumed.cpp << \EOF
/*
*******************************************************************************
*                                                                             *
*                                PLUMED                                       *
...
EOF
cat >./USER-PLUMED/fix_plumed.h  << \EOF
/*
*******************************************************************************
*                                                                             *
*                                PLUMED                                       *
...
cat >./USER-PLUMED/style_user_plumed.h << \EOF
#ifdef FixInclude
#include "fix_plumed.h"
#endif
...
#awk '/user-imd user-smd =/{print $0," user-plumed"}!/user-imd user-smd =/{print}' Makefile.preplumed > Makefile
sed 's/PACKUSER =/PACKUSER = user-plumed /' Makefile.preplumed > Makefile
#("make makelist" version 07Nov09 does not work due to sed not accepting long strings, here down alternative command)
awk '/^SRC =/{system("echo SRC = `ls *.cpp`")}
     /^INC =/{system("echo INC = `ls *.h`")}  
     !(/^SRC =/||/^INC =/){print}' Makefile.list > l; mv l Makefile.list
echo '#include "style_user_plumed.h"' >> style_user_packages.h
sed s/"CCFLAGS \="/"CCFLAGS \=  -DLAMMPS_PLUMED"/ ./MAKE/Makefile.$myarch > l ;
cp   ./MAKE/Makefile.$myarch  ./MAKE/Makefile.${myarch}.preplumed; 
mv l ./MAKE/Makefile.$myarch 
...
function to_do_after_revert () {
  ...
  awk '/SRC =/{system("echo SRC = `ls *.cpp`")}
       /INC =/{system("echo INC = `ls *.h`")}  
       !/[SRC =][INC =]/{print}' Makefile.list > l; mv l Makefile.list
  grep -v style_user_plumed style_user_packages.h > l;mv l style_user_packages.h
  mv Makefile.preplumed Makefile
  mv  ./MAKE/Makefile.${myarch}.preplumed ./MAKE/Makefile.$myarch 
}
