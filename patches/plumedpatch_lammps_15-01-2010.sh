...
if (test $1 = 1) then
  ...
  cp -p style_user_plumed.h ../
  cp -p metadyn.h ../
  cp -p fix_plumed.h ../
  cp -p testderivatives.cpp ../
  cp -p restraint_zpath.cpp ../
  cp -p restraint_waterbridge.cpp ../
  cp -p restraint_torsion.cpp ../
  cp -p restraint_spath.cpp ../
  cp -p restraint_rmsdtor.cpp ../
  cp -p restraint_rgyr.cpp ../
  cp -p restraint_puckering.cpp ../
  cp -p restraint_position.cpp ../
  cp -p restraint_parabetarmsd.cpp ../
  cp -p restraint_mindist.cpp ../
  cp -p restraint_helix.cpp ../
  cp -p restraint_hbonds.cpp ../
  cp -p restraint_energy.cpp ../
  cp -p restraint_elstpot.cpp ../
  cp -p restraint_dist.cpp ../
  cp -p restraint_dipole.cpp ../
  cp -p restraint_dihcor.cpp ../
  cp -p restraint_coord.cpp ../
  cp -p restraint_cmap.cpp ../
  cp -p restraint_antibetarmsd.cpp ../
  cp -p restraint_angle.cpp ../
  cp -p restraint_alpharmsd.cpp ../
  cp -p restraint_alfabeta.cpp ../
  cp -p restraint.cpp ../
  cp -p read_restraint.cpp ../
  cp -p ptmetad.cpp ../
  cp -p metadyn.cpp ../
  cp -p hills.cpp ../
  cp -p fix_plumed.cpp ../
  cp -p biasexchange.cpp ../

elif (test $1 = 0) then

  rm ../style_user_plumed.h
  rm ../metadyn.h
  rm ../fix_plumed.h
  rm ../testderivatives.cpp
  rm ../restraint_zpath.cpp
  rm ../restraint_waterbridge.cpp
  rm ../restraint_torsion.cpp
  rm ../restraint_spath.cpp
  rm ../restraint_rmsdtor.cpp
  rm ../restraint_rgyr.cpp
  rm ../restraint_puckering.cpp
  rm ../restraint_position.cpp
  rm ../restraint_parabetarmsd.cpp
  rm ../restraint_mindist.cpp
  rm ../restraint_helix.cpp
  rm ../restraint_hbonds.cpp
  rm ../restraint_energy.cpp
  rm ../restraint_elstpot.cpp
  rm ../restraint_dist.cpp
  rm ../restraint_dipole.cpp
  rm ../restraint_dihcor.cpp
  rm ../restraint_coord.cpp
  rm ../restraint_cmap.cpp
  rm ../restraint_antibetarmsd.cpp
  rm ../restraint_angle.cpp
  rm ../restraint_alpharmsd.cpp
  rm ../restraint_alfabeta.cpp
  rm ../restraint.cpp
  rm ../read_restraint.cpp
  rm ../ptmetad.cpp
  rm ../metadyn.cpp
  rm ../hills.cpp
  rm ../fix_plumed.cpp
...
//  printf("PROCESSOR %d SIZE %d\n",me,size);
  ...
  //   printf("FOUND MASS  %f DT %f\n",group->mass(0),update->dt);
     ...
     for(i=0;i<nn;i++){chg[i]=-100.0;} 
     for(i=0;i<nn;i++){mss[i]=-100.0;} 
     //use the groups for massses
     MPI_Request request;
     int groupbit = group->bitmask[0]; //get the groupbit of all atoms
...
//             printf("I AM PROC %d AND I OWN AT %d R-MASS %f TAG %d \n",me,mm,mss[mm],atom->tag[i]);
             MPI_Isend(&(mss[mm]),1,MPI_DOUBLE,0,mm,world,&request);
        }
      }
     } else {
        for (int i = 0; i < atom->nlocal; i++){
...
//     printf("ME: %dEVERYONE SENT \n",me); 
     ...
     if(!me){
        MPI_Status status;
        for (i=0;i<nn;i++){
          if(mss[i]==-100.0)MPI_Recv(&(mss[i]),1,MPI_DOUBLE,MPI_ANY_SOURCE,i,world,&status); 
        }
     }
...
//             printf("I AM PROC %d AND I OWN AT %d CHG %f \n",me,mm,chg[mm]);
             MPI_Isend(&(chg[mm]),1,MPI_DOUBLE,0,mm,world,&request);
        }
     }
...
//     printf("ME: %dEVERYONE SENT \n",me); 
     ...
     if(!me){
        MPI_Status status;
        for (i=0;i<nn;i++){
          if(chg[i]==-100.0)MPI_Recv(&(chg[i]),1,MPI_DOUBLE,MPI_ANY_SOURCE,i,world,&status); 
        }
     }
...
//      printf("LOCAL ATOM %d :I BELONG TO PLUMED IN PROC %d TAG: %d\n",i,me,tag[i]);
 ...
 //     printf("PROC %d IDX %d LOCAL_IDX %d\n",me, my_idx[j],my_backtable[j]);
  ...
  int sum_allatoms;
  MPI_Allreduce(&my_atoms[me],&sum_allatoms,1,MPI_INT,MPI_SUM,world);  
  MPI_Allgather(&my_atoms[me],1,MPI_INT,my_atoms,1,MPI_INT,world);  
  // test the allgather 
